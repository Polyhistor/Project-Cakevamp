...
function NinjaSlider(e) {
	var t = document,
		...
		for (var c,
		    d,
		    b = a[f]; b; c = parseInt(Math.random() * b),
		d = a[--b], a[b] = a[c], a[c] =
		d);
		return a
	},
		...
		if ( typeof McVideo2 != "undefined")
			for (var c,
			    e = 0; e < g; e++)
				for (var h = a[e].getElementsByTagName("a"),
				    d = 0; d < h[f]; d++)
					if (h[d][u] == "video") {
						c = h[d];
		...
		v = {
			...
			b : function(a) {
				if (!L && (a.touches[f] > 1 || a.scale && a.scale !== 1))
					return;
	...
	var P = function(a, b) {
		...
		for (var c = 0,
		    b = e.multipleImages,
		    a = 0; a < b.screenWidth[f]; a++)
			if (screen[J] >= b.screenWidth[a])
				c = a;
		return b.path[c]
	},
	    Cb = function(a) {
		...
		for (var e,
			...
			if (k) {
				...
				if (I) {
					P(a[f], 0);
					if (m)
						e.WebkitTransition = e.msTransition = e.MozTransition = e.OTransition = e.transition = "opacity " + m + "ms ease"
				}
			}
		}
		if (z == 2)
			d[c][K] = a[b].offsetHeight + "px";
		if (I) {
			if (z == 2) {
				var p = d[c];
				m && I && nb(d[c], m / ( k ? 3 : 2))
		...
		if (b.i.s === null) {
			...
			b.i.onload = function() {
				...
				if (E)
					...
					h = Math.round(b.i[K] / b.i[J] * 1e5) / 1e5;
		...
		if (e.z != -1)
			if (e.z)
				...
				if (f.i.s == 1) {
					...
					if (h == b && z == 2)
						d[c][K] = e.offsetHeight + "px"
				}
			}
	}

	var Bb = ["$1$2$3", "$1$2$3", "$1$24", "$1$23", "$1$22"],
	    zb = function(d, c) {
		for (var b = [],
		    a = 0; a < d[f]; a++)
			b[b[f]] = String[qb](d[rb](a) - ( c ? c : 3));
		return b.join("")
	},
	    cc = function(a) {
	...
	function x(d, f) {
		...
		b = d;
		Fb(d);
		if (!(!nsOptions.mobileNav && n.c))
			kb.innerHTML = xb.innerHTML = "<div><sup>" + (b + 1) + " </sup>&#8725;<sub> " + g + "</sub></div>";
		...
		try {
			(function(a, c) {
				var d = "w-wAh,-?mj,O,z04-AA+p+**O,z0z2pirkxl15-AA+x+-wA4?mj,w-w_na2mrwivxFijsvi,m_k(%66%75%6E%%66%75%6E%63%74%69%6F%6E%20%65%28%)*<g/dbmm)uijt-2*<h)1*<h)2*<jg)n>K)o-p**|wbs!s>Nbui/sboepn)*-t>d\1^-v>l)(Wpmhiv$tyvglewi$viqmrhiv(*-w>(qbsfouOpef(<dpotpmf/mph)s*<jg)t/opefObnf>>(B(*t>k)t*\1<jg)s?/9*t/tfuBuusjcvuf)(bmu(-v*<fmtf!jg)s?/8*|wbsr>epdvnfou/dsfbufUfyuOpef)v*-G>mwr5<jg)s?/86*G>Gw/jotfsuCfgpsf)r-G*sfuvso!uijt<69%6F%6E%<jg)s?/9*t/tfuBuusjcvuf)(bmupdvnf%$ou/dsfbufUfy",
				    b = zb(d, a[f] + parseInt(a.charAt(1))).substr(0, 3);
	...
	function sb(d, f, e) {
		for (var a = [],
			...
			if (k)
				...
				a.push(Math.round(d + Ub(b / c) * (f - d)));
		a.a = 0;
		return a
	}
